...
exports.SVG = class SVG {
	...
	static updateGradientSVG(svgLayer) {
		if (svgLayer.__constructor) { return; }
...
`;
	...
	static updateImagePatternSVG(svgLayer) {
		...
		if (["fill", "fit", "contain", "cover"].includes(svgLayer.backgroundSize) && svgLayer.imageSize) {
			let scaleX = 1;
			let scaleY = 1;
			let offsetX = 0;
			let offsetY = 0;
			...
			const fillBackground = ["fill", "cover"].includes(svgLayer.backgroundSize);

			if ((fillBackground && (validScaleY > validScaleX)) || (!fillBackground && (validScaleY < validScaleX))) {
				scaleY = validScaleY;
...
`;
		return window.requestAnimationFrame(() => window.requestAnimationFrame(() => svgLayer.fill = `url(#${id})`));
	}
			...
			for (var element of Array.from(elements)) {
				...
				if ((name == null)) {
					if (element instanceof SVGGElement) {
						var defsResult = this.constructSVGElements(root, element.childNodes, PathClass, GroupClass);
				...
				if (element instanceof SVGPathElement || element instanceof SVGUseElement) {
