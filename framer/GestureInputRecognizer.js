...
exports.GestureInputRecognizer = class GestureInputRecognizer {
	...
	shouldFireTapEvent(event) {
		const startEvent = this.session != null ? this.session.startEvent : undefined;
		if (startEvent != null) {
			...
			const isShortDistance = Utils.pointDistance(startEvent.touchCenter, event.touchCenter) < 45;
			return isSameTarget && isShortSession && isShortDistance && !this.session.cancelTap;
		} else {
			return true;
		}
	}
	...
	edgeswipedirection(event) {
		const swipeEdge = this._edgeForSwipeDirection(this.session.started.edgeswipedirection.offsetDirection);
	...
	_getGestureEvent(event) {
		...
		for (var pointKey of ["point", "start", "previous", "offset", "delta", "velocity", "touchCenter", "touchOffset"]) {
