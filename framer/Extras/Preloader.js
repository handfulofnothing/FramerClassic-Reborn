...
class Preloader extends BaseClass {
	static initClass() {
		...
		this.define("progress",
			{get() { return (this._mediaLoaded.length / this._media.length) || 0; }});
	
		this.define("time",
			{get() { return (Date.now() - this._startTime) / 1000; }});
	...
	setLogo(url) {
		...
		if (this.brand) { return this.brand.style["background-image"] = `url('${url}')`; }
	}
	...
	addImage(image) {
		if (image instanceof Gradient) { return; }
	...
	addPlayer(player) {
		if (!player || Array.from(this._media).includes(player)) { return; }
		if ((player.src == null) || (player.getAttribute('src') === "undefined")) { return; }
		if ((player.readyState == null) || !(player.readyState < 3)) { return; }
		// … else
	...
	_end(animated) {
		...
		if ((this.progressIndicator != null ? this.progressIndicator.visible : undefined) && animated) {
			if (this.cover != null) {
				this.cover.animate({
	...
	_handleProgress() {
		...
		if (this.isReady) { return this._handleLoaded(); }
	}
	...
	_setupContext() {
		...
		if (this._logo) {
			...
			let logoUrl = "//resources.framerjs.com/static/images/preloader/framer-logo.png";
			if (_.startsWith(window.location.href, "file://")) { logoUrl = "http:" + logoUrl; }
...
exports.enable = () => Framer.Preloader != null ? Framer.Preloader : (Framer.Preloader = new Preloader());

exports.disable = function() {
	if (!Framer.Preloader) { return; }
	Framer.Preloader._end(false);
	return Framer.Preloader = null;
};

exports.addImage = url => Framer.Preloader != null ? Framer.Preloader.addImage(url) : undefined;

exports.setLogo = url => Framer.Preloader != null ? Framer.Preloader.setLogo(url) : undefined;
