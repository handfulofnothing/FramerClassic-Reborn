...
const Cls = (exports.BaseClass = class BaseClass extends EventEmitter {
	static initClass() {
		...
		this.define("props", {
			...
			set(value) {
				...
				return (() => {
					...
					// importable.
						var v = value[k];
						if (propertyList[k] != null ? propertyList[k].importable : undefined) { result.push(this[k] = v); } else {
	...
	static undefine(propertyName) {
		if (_.isArray(propertyName)) {
			...
			return this.define(propertyName, this.simpleProperty(propertyName, undefined, {
				importable: false,
				exportable: false,
				enumerable: false
			}
	...
	static _addDescriptor(propertyName, descriptor) {
		...
		if (descriptor.readonly == null) { descriptor.readonly = (descriptor.set == null); }
	...
	_applyDefaults(options, proxy) {
		...
		return (() => {
			...
			for (var k of Array.from(DefaultPropertyOrder)) {
				...
				if (descriptor != null) {
					if (proxy && !(descriptor.proxy === true)) { continue; }
