...
const Cls = (exports.LayerDraggable = class LayerDraggable extends BaseClass {
	static initClass() {
		...
		this.define("constraintsOffset", {
			get() {
				...
				const {minX, maxX, minY, maxY} = this._calculateConstraints(this.constraints);
				const {
                    point
                } = this.layer;
		...
		this.define("direction", {
			get() {
				...
				if ((velocity.x === 0) && (velocity.y === 0)) {
					const delta = this._lastEvent != null ? this._lastEvent.delta : undefined;
	...
	tapStart(event) {
		if (this._isAnimating) {
			__guard__(Framer.GestureInputRecognizer != null ? Framer.GestureInputRecognizer.session : undefined, x => x.cancelTap = true);
			return this._panStart(event);
		}
	}
	...
	_panMove(event) {
		...
		if (this.pixelAlign) {
			if (this.horizontal) { point.x = Math.round(point.x); }
			if (this.vertical) { point.y = Math.round(point.y); }
		}
	...
	_constrainPosition(proposedPoint, bounds, scale) {
		...
		let point;
		const {minX, maxX, minY, maxY} = this._calculateConstraints(this._constraints);
	...
	_onSimulationStop(axis, state) {
		...
		if (this.pixelAlign) { this.layer[axis] = Math.round(this.layer[axis]); }
	...
	_startSimulation() {
		...
		const startSimulationX = (this.overdrag === true) || ((this.layer.x > minX) && (this.layer.x < maxX));
		const startSimulationY = (this.overdrag === true) || ((this.layer.y > minY) && (this.layer.y < maxY));
		...
		const velocityX = velocity.x * this.momentumVelocityMultiplier * this.speedX * (1 / this.layer.canvasScaleX()) * this.layer.scaleX * this.layer.scale;
		const velocityY = velocity.y * this.momentumVelocityMultiplier * this.speedY * (1 / this.layer.canvasScaleY()) * this.layer.scaleY * this.layer.scale;
