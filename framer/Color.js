...
const Cls = (exports.Color = class Color extends BaseClass {
	static initClass() {
	...
	toHusl() {
		if (this._husl === undefined) {
			const c = libhusl._conv;
			const husl = c.lch.husl(c.luv.lch(c.xyz.luv(c.rgb.xyz([this.r/255, this.g/255, this.b/255]))));
	...
	static gray(...args) { return this.grey(...Array.from(args || [])); }
	...
	static isValidColorProperty(name, value) {
		...
		const isColorKey = _.endsWith(name.toLowerCase(), "color") || ["fill", "stroke"].includes(name);
	...
	static equal(colorA, colorB) {
		...
		if (Math.abs(colorA.a - colorB.a) >= tolerance) { return false; }
		return true;
	}
...
ColorModel.isRGB = function(colorModel) {
	if (_.isString(colorModel)) { let needle;
	return (needle = colorModel.toLowerCase(), [ColorModel.RGB, ColorModel.RGBA].includes(needle)); }
	return false;
};

ColorModel.isHSL = function(colorModel) {
	if (_.isString(colorModel)) { let needle;
	return (needle = colorModel.toLowerCase(), [ColorModel.HSL, ColorModel.HSLA].includes(needle)); }
	return false;
};

var rgbaFromHusl = function(husl) {
	const c = libhusl._conv;
	const rgb = c.xyz.rgb(c.luv.xyz(c.lch.luv(c.husl.lch([husl.h, husl.s, husl.l]))));
	const rgba = {
		r: rgb[0] * 255,
		g: rgb[1] * 255,
		b: rgb[2] * 255,
