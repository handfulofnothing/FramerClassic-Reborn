...
Utils.setValueForKeyPath = function(obj, path, val) {
	...
	while ((i < n) && (result !== undefined)) {
		var field = fields[i];
		if (i === (n - 1)) {
			var currentValue = result[field];
			if (_.isObject(currentValue) && _.isObject(val) && (Object.getPrototypeOf(currentValue) === Object.prototype) && (Object.getPrototypeOf(val) === Object.prototype)) {
...
Utils.webkitPerspectiveForValue =  function(value) {
	if (["none", null, 0].includes(value)) {
		return "none";
...
Utils.memoize = fn => (function() {
	...
	if (hash in fn.memoize) { return fn.memoize[hash]; } else { return fn.memoize[hash] = fn.apply(this, args); }
});
...
Utils.findLayer = (layers, selector) => _.find(layers, layer => Utils.layerMatchesSelector(layer, selector));

Utils.filterLayers = (layers, selector) => _.filter(layers, layer => Utils.layerMatchesSelector(layer, selector));

Utils.layerMatchesSelector = function(layer, selector) {
	const getHierarchyString = function(l) {
	...
	const hierarchyMatch = function(hierarchy, string) {
...
Utils.cycle = function() {
	...
	const args = Utils.arrayFromArguments(arguments);
...
Utils.equal = function(a, b) {
	if (_.isFunction(a != null ? a.isEqual : undefined)) {
		return a.isEqual(b);
	}
	if (_.isFunction(b != null ? b.isEqual : undefined)) {
		return b.isEqual(a);
	}
...
Utils.isFirefox = () => /^Mozilla.*Firefox\/\d+\.\d+$/.test(navigator.userAgent);
...
Utils.isTouch = () => (window.ontouchstart === null) &&
...
Utils.isLocalServerUrl = url => /[a-zA-Z]{1,8}:\/\/127\.0\.0\.1/.test(url) || /[a-zA-Z]{1,8}:\/\/localhost/.test(url);
...
Utils.framerStudioVersion = function() {
	...
	if (Utils.isFramerStudio()) {
		...
		let version;
		const isBeta = navigator.userAgent.indexOf("FramerStudio/beta") >= 0;
		const isLocal = navigator.userAgent.indexOf("FramerStudio/local") >= 0;
		const isFuture = navigator.userAgent.indexOf("FramerStudio/future") >= 0;
...
Utils.deviceFont = function(os) {
	...
	const appleFont = "-apple-system, BlinkMacSystemFont, SF UI Text, Helvetica Neue";
	const googleFont = "Roboto, Helvetica Neue";
	const microsoftFont = "Segoe UI, Helvetica Neue";
	switch (os) {
		case "Android": return googleFont; break;
		case "iOS": case "watchOS": case "macOS": return appleFont; break;
		case "Windows": return microsoftFont; break;
	}
...
const getWidth = fontFamily => Utils.textSize("BESbswy", {
    fontFamily,
    fontSize: 300
}).width;
...
Utils.isFontAvailable = function(fonts) {
	if (_isFontLoadedResults[fonts] === true) {
		return true;
	}
	...
	if ((monoWidth !== getWidth(fonts + ",monospace")) || (serifWidth !== getWidth(fonts + ",serif")) || (sansWidth !== getWidth(fonts + ",sans-serif"))) {
...
Utils.isFontFamilyLoaded = function(fonts, timeout) {
	if (timeout == null) { timeout = 1000; }
	if (!_.isArray(fonts)) {
		fonts = [fonts];
	}
	const unavailableFonts = fonts.filter(font => !Utils.isFontAvailable(font));
...
const fontsFromConfig = function(config) {
	let result = [];
	if (_.isArray(__guard__(config != null ? config.custom : undefined, x => x.families))) {
		result = result.concat(__guard__(config != null ? config.custom : undefined, x1 => x1.families));
	}
	if (_.isArray(__guard__(config != null ? config.google : undefined, x2 => x2.families))) {
		result = result.concat(__guard__(config != null ? config.google : undefined, x3 => x3.families));
	}
	return result;
};

Utils.loadWebFontConfig = function(config) {
	const fonts = fontsFromConfig(config);
	let allLoadedResult = null;
	for (var currentFont of Array.from(fonts)) {
		var currentFontLoaded = _isFontLoadedResults[currentFont];
		if ((currentFontLoaded == null)) {
			allLoadedResult = null;
			break;
		}
		if (allLoadedResult == null) { allLoadedResult = currentFontLoaded; }
		allLoadedResult = allLoadedResult && currentFontLoaded;
	}
	if (allLoadedResult != null) {
		return allLoadedResult;
	}

	const customActive = config.active;
	const customInactive = config.inactive;
	const customFontactive = config.fontactive;
	const customFontinactive = config.fontinactive;

	const promise = new Promise(function(resolve, reject) {
		config.fontactive = function(font) {
			_isFontLoadedResults[font] = true;
			if (typeof customFontactive === 'function') {
		...
		config.inactive = function() {
			if (typeof customInactive === 'function') {
				customInactive();
			}
			const error = new Error(`${fonts.join(', ')} failed to load`);
			return reject(error);
		};
...
Utils.loadWebFont = function(font, weight, source) {
	if (source == null) { source = "google"; }
	if ((_isFontLoadedResults[font] == null) || (_isFontLoadedResults[font] === false)) {
...
Utils.round = function(value, decimals, increment=null, min=null, max=null) {
...
Utils.domLoadJSON = (path, callback) => Utils.domLoadData(path, (err, data) => callback(err, JSON.parse(data)));
...
Utils.isInsideIframe = function() {
	if (!Utils.isInsideFramerCloud()) { return window !== window.top; }
	return false;
};

Utils.isInsideFramerCloud = () => Utils.getQueryParameters()["cloud"] === "1";

Utils.getQueryParameters = () => _.fromPairs(window.location.search.slice(1).split('&').map(val => val.split('=')));
...
Utils.pointDivide = (point, fraction) => point = {
    x: point.x / fraction,
    y: point.y / fraction
};
...
Utils.framePointForOrigin = (frame, originX, originY) => frame = {
...
Utils.pointInPolygon = function(point, vs) {
	...
	while (i < vs.length) {
		...
		var xj = vs[j][0];
		var yj = vs[j][1];
		var intersect = (yi > y && y !== yj && yj > y) && (x < ((((xj - xi) * (y - yi)) / (yj - yi)) + xi));
		if (intersect) {
			inside = !inside;
		}
		j = i++;
...
Utils.convertPointToContext = function(point, layer, rootContext, includeLayer) {
	...
	if (includeLayer) { containers.unshift(layer); }

	for (var container of Array.from(containers)) {
		if (container.flat || container.clip) { point.z = 0; }
...
Utils.convertFrameToContext = function(frame, layer, rootContext, includeLayer) {
	...
	const convertedCorners = corners.map(point => Utils.convertPointToContext(point, layer, rootContext, includeLayer));
...
Utils.convertPointFromContext = function(point, layer, rootContext, includeLayer) {
	...
	const containers = layer.containers(rootContext);
	containers.reverse();
	if (includeLayer) { containers.push(layer); }
...
Utils.convertFrameFromContext = function(frame, layer, rootContext, includeLayer) {
	...
	frame = _.defaults(frame, {x: 0, y: 0, width: Framer.Defaults.Layer.width, height: Framer.Defaults.Layer.height});
	const corners = Utils.pointsFromFrame(frame);
	const convertedCorners = corners.map(point => Utils.convertPointFromContext(point, layer, rootContext, includeLayer));
	return Utils.frameFromPoints(convertedCorners);
};
...
Utils.boundingFrame = function(layer, rootContext) {
	...
	const contextCornerPoints = cornerPoints.map(point => Utils.convertPointToContext(point, layer, rootContext));
...
Utils.escapeForRegex = string => string.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
