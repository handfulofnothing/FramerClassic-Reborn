...
const getShadowStrings = function(layer, types, createString) {
	...
	if (layer.shadows != null) {
		for (var shadow of Array.from(layer.shadows)) {
			...
			if (!(Array.from(types).includes(shadow.type)) || ((shadow.x === 0) && (shadow.y === 0) && (shadow.blur === 0) && (shadow.spread === 0))) {
				continue;
			}
			if (shadow.color === null) {
				shadow.color = new Color(null);
			}
			var dpr = layer._pixelMultiplierOverride != null ? layer._pixelMultiplierOverride : layer.context.pixelMultiplier;
...
exports.LayerStyle = {
	...
	width(layer) {
		...
		const dpr = layer._pixelMultiplierOverride != null ? layer._pixelMultiplierOverride : layer.context.pixelMultiplier;
	...
	webkitFilter(layer) {
		...
		let css = [];
		const dpr = layer._pixelMultiplierOverride != null ? layer._pixelMultiplierOverride : layer.context.pixelMultiplier;
		for (var [cssName, layerPropertyName, fallback, unit] of Array.from(_WebkitProperties)) {
			if (layer._properties.hasOwnProperty(layerPropertyName) && (layer[layerPropertyName] !== fallback)) {
		...
		// filter shadow
		const shadowStrings = getShadowStrings(layer, "drop", (shadow, pixelMultiplier) => `drop-shadow(${shadow.x * pixelMultiplier}px ${shadow.y * pixelMultiplier}px ${shadow.blur * pixelMultiplier}px ${shadow.color})`);
	...
	webkitBackdropFilter(layer) {
		const css = [];
		const dpr = layer._pixelMultiplierOverride != null ? layer._pixelMultiplierOverride : layer.context.pixelMultiplier;
		for (var [cssName, layerPropertyName, fallback, unit] of Array.from(_BackdropProperties)) {
			if (layer._properties.hasOwnProperty(layerPropertyName) && (layer[layerPropertyName] !== fallback)) {
				var filter = filterFormat(cssName, layer[layerPropertyName], unit, dpr);
				css.push(filter);
			}
...
`;
	...
	webkitPerspective(layer) {
		let left;
		const value = (left = Utils.webkitPerspectiveForValue(layer._properties.perspective)) != null ? left : "";
		if (_.isNumber(value)) {
			const dpr = layer._pixelMultiplierOverride != null ? layer._pixelMultiplierOverride : layer.context.pixelMultiplier;
			return `${value * dpr}`;
		} else {
	...
	mixBlendMode(layer) {
		let needle;
		if ((needle = layer._properties.blending, Array.from(_.values(Blending)).includes(needle))) {
	...
	boxShadow(layer) {
		const shadowStrings = getShadowStrings(layer, ["box", "inset"], function(shadow, pixelMultiplier) {
			const insetString = shadow.type === "inset" ? "inset " : "";
			return `${insetString}${shadow.x * pixelMultiplier}px ${shadow.y * pixelMultiplier}px ${shadow.blur * pixelMultiplier}px ${shadow.spread * pixelMultiplier}px ${shadow.color}`;
		});
	...
	textShadow(layer) {
		const shadowStrings = getShadowStrings(layer, "text", (shadow, pixelMultiplier) => `${shadow.x * pixelMultiplier}px ${shadow.y * pixelMultiplier}px ${shadow.blur * pixelMultiplier}px ${shadow.color}`);
		return shadowStrings.join(", ");
	},
	...
	backgroundSize(layer) {
		switch (layer._properties.backgroundSize) {
			case "fill": return "cover"; break;
			case "fit": return "contain"; break;
			case "stretch": return "100% 100%"; break;
		}
	...
	strokeWidth(layer) {
		const factor = layer.strokeWidthMultiplier != null ? layer.strokeWidthMultiplier : 1;
	...
	borderRadius(layer) {
		...
		if (_.isObject(layer._properties.borderRadius)) {
			return ((radius.topLeft != null ? radius.topLeft : 0) * dpr) + "px " + ((radius.topRight != null ? radius.topRight : 0) * dpr) + "px " + ((radius.bottomRight != null ? radius.bottomRight : 0) * dpr) + "px " + ((radius.bottomLeft != null ? radius.bottomLeft : 0) * dpr) + "px";
		}
	...
	borderWidth(layer) {
		...
		if (_.isNumber(borderWidth)) {
			let left1;
			const borderTopBottom = ((left = Math.min(borderWidth, layer.height / 2)) != null ? left : 0) * dpr;
			const borderRightLeft = ((left1 = Math.min(borderWidth, layer.width / 2)) != null ? left1 : 0) * dpr;
	...
	direction(layer) {
		...
		switch (value) {
			case "right-to-left": return "rtl";
			case "left-to-right": return "ltr";
