...
const {LayerStates} = require("./LayerStates");
const {LayerDraggable} = require("./LayerDraggable");
const {LayerPinchable} = require("./LayerPinchable");
const {Gestures} = require("./Gestures");
const {LayerPropertyProxy} = require("./LayerPropertyProxy");

const NoCacheDateKey = Date.now();

const delayedStyles = ["webkitTransform", "webkitFilter", "webkitPerspectiveOrigin", "webkitTransformOrigin", "webkitBackdropFilter"];

const layerValueTypeError = function(name, value) {
	throw new Error(`Layer.${name}: value '${value}' of type '${typeof(value)}' is not valid`);
};

const layerProperty = function(obj, name, cssProperty, fallback, validator, transformer, options, set, targetElement, includeMainElement, useSubpropertyProxy) {
	...
	let result = {
		...
		get() {
			...
			let value;
			if (this._properties.hasOwnProperty(name)) { value = this._properties[name]; }
			if (value == null) { value = fallback; }

			if (useSubpropertyProxy) { return layerProxiedValue(value, this, name); }
			return value;
		},
		...
		set(value) {
			...
			if (cssProperty !== null) {
				...
				if (includeMainElement || !targetElement) { mainElement = elementContainer._element; }
				if (targetElement != null) { subElement = elementContainer[targetElement]; }
				...
				} else if (!this.__applyingDefaults || (!Array.from(delayedStyles).includes(cssProperty))) {
...
var layerProxiedValue = function(value, layer, property) {
	if (!_.isObject(value)) { return value; }
	return new LayerPropertyProxy(value, function(proxiedValue, subProperty, subValue) {
...
const Cls = (exports.Layer = class Layer extends BaseClass {
	static initClass() {
		...
		// Css properties
		this.define("width",  layerProperty(this, "width", "width", 100, _.isNumber, null, {}, function(layer, value) {
			if ((layer.constraintValues == null) || layer.isLayouting) { return; }
		...
		this.define("height", layerProperty(this, "height", "height", 100, _.isNumber, null, {}, function(layer, value) {
			if ((layer.constraintValues == null) || layer.isLayouting) { return; }
		...
		this.define("opacity", layerProperty(this, "opacity", "opacity", 1, _.isNumber, parseFloat));
		this.define("index", layerProperty(this, "index", "zIndex", 0, _.isNumber, null, {importable: false, exportable: false}));
		this.define("clip", layerProperty(this, "clip", "overflow", false, _.isBoolean, null, {}, null, "_elementHTML", true));
		...
		this.define("x", layerProperty(this, "x", "webkitTransform", 0, _.isNumber,
			layerPropertyPointTransformer, {depends: ["width", "height"]}, function(layer) {
				if (layer.isLayouting) { return; }
		...
		this.define("perspective", layerProperty(this, "perspective", "webkitPerspective", 0, (v => Utils.webkitPerspectiveForValue(v) !== null)));
		...
		this.define("blending", layerProperty(this, "blending", "mixBlendMode", null, _.isString));
	
		this.define("backgroundBlur", layerProperty(this, "backgroundBlur", "webkitBackdropFilter", 0, _.isNumber));
		this.define("backgroundBrightness", layerProperty(this, "backgroundBrightness", "webkitBackdropFilter", 100, _.isNumber));
		this.define("backgroundSaturate", layerProperty(this, "backgroundSaturate", "webkitBackdropFilter", 100, _.isNumber));
		this.define("backgroundHueRotate", layerProperty(this, "backgroundHueRotate", "webkitBackdropFilter", 0, _.isNumber));
		this.define("backgroundContrast", layerProperty(this, "backgroundContrast", "webkitBackdropFilter", 100, _.isNumber));
		this.define("backgroundInvert", layerProperty(this, "backgroundInvert", "webkitBackdropFilter", 0, _.isNumber));
		this.define("backgroundGrayscale", layerProperty(this, "backgroundGrayscale", "webkitBackdropFilter", 0, _.isNumber));
		this.define("backgroundSepia", layerProperty(this, "backgroundSepia", "webkitBackdropFilter", 0, _.isNumber));
	
		this.define("backgroundSize", layerProperty(this, "backgroundSize", "backgroundSize", "fill", _.isString));
		...
		for (let i = 0; i <= 8; i++) {
			(i => {
				return this.define(`shadow${i+1}`, {
					...
					get() {
						if (this.shadows == null) { this.shadows = []; }
						if (this.shadows[i] == null) { this.shadows[i] = proxiedShadowValue(this, {}, i); }
		...
		for (var shadowProp of ["X", "Y", "Blur", "Spread", "Color", "Type"]) {
			(shadowProp => {
				return this.define(`shadow${shadowProp}`, {
					get() {
						if ((this.shadows == null) || (this.shadows.length === 0)) { return null; }
		...
		this.define("color", layerProperty(this, "color", "color", null, Color.validColorValue, Color.toColor, null, null, "_elementHTML", true));
	
		// Border properties
		this.define("borderRadius", layerProperty(this, "borderRadius", "borderRadius", 0, null, asBorderRadius, null, null, "_elementBorder", true, true));
		this.define("borderColor", layerProperty(this, "borderColor", "borderColor", null, Color.validColorValue, Color.toColor, null, null, "_elementBorder"));
		this.define("borderWidth", layerProperty(this, "borderWidth", "borderWidth", 0, null, asBorderWidth, null, null, "_elementBorder", false, true));
		this.define("borderStyle", layerProperty(this, "borderStyle", "borderStyle", "solid", _.isString, null, null, null, "_elementBorder"));
		...
		this.define("constraintValues", {
			...
			exportable: false,
			default: null,
			get() { return this._getPropertyValue("constraintValues"); },
			set(value) {
				let newValue;
				if (value === null) {
					newValue = null;
					...
					if (this.parent != null) {
						let needle, needle1;
						if (!((needle = this.layout, Array.from(this.parent.listeners("change:width")).includes(needle)))) {
							this.parent.on("change:width", this.layout);
						}
						if (!((needle1 = this.layout, Array.from(this.parent.listeners("change:height")).includes(needle1)))) {
							this.parent.on("change:height", this.layout);
						}
					} else {
						let needle2;
						if (!((needle2 = this.layout, Array.from(Screen.listeners("resize")).includes(needle2)))) {
							Screen.on("resize", this.layout);
						}
					}
					if (!((needle3 = this.parentChanged, Array.from(this.listeners("change:parent")).includes(needle3)))) {
		...
		this.define("htmlIntrinsicSize", {
			...
			exportable: true,
			default: null,
			get() { return this._getPropertyValue("htmlIntrinsicSize"); },
			set(value) {
				if (value === null) {
					return this._setPropertyValue("htmlIntrinsicSize", value);
				} else {
					if (!_.isFinite(value.width) || !_.isFinite(value.height)) { return; }
					return this._setPropertyValue("htmlIntrinsicSize", {width: value.width, height: value.height});
				}
			}
		});
		...
		this.define("image", {
			...
			set(value) {
				...
				const isBackgroundColorDefault = this.backgroundColor != null ? this.backgroundColor.isEqual(defaults.backgroundColor) : undefined;
				...
				if (Gradient.isGradientObject(value)) {
					...
					if (isBackgroundColorDefault) { this.backgroundColor = null; }
					return;
				}
		...
		this.define("gradient", {
			get() {
				if (Gradient.isGradientObject(this.image)) { return layerProxiedValue(this.image, this, "gradient"); }
				return null;
			},
		...
		this.define("children", {
			...
			exportable: false,
			importable: false,
			get() { return this._children.map(function(c) {
				if (c instanceof SVGLayer && (c.children.length === 1) && _.startsWith(c.name, '.')) {
					return c.children[0];
				} else {
					return c;
				}
	...
	constructor(options) {
		...
		if (this._stylesAppliedToParent == null) { this._stylesAppliedToParent = []; }
	...
	static ExistingIdMessage(type, id) { return `Can not set ${type}: There's already an element with id '${id}' in this document'`; }
	...
	updateShadowStyle() {
		...
		return this._element.style.webkitFilter = LayerStyle["webkitFilter"](this);
	}
	...
	_layoutX() {
		if ((this.constraintValues == null)) { return; }
		if ((this.parent == null) && !this.context.autoLayout) { return; }
		const parentFrame = (this.parent != null ? this.parent.frame : undefined) != null ? (this.parent != null ? this.parent.frame : undefined) : this.context.innerFrame;
		this.isLayouting = true;
		this.x = Utils.calculateLayoutX(parentFrame, this.constraintValues, this.width);
	...
	_layoutY() {
		...
		const parentFrame = (this.parent != null ? this.parent.frame : undefined) != null ? (this.parent != null ? this.parent.frame : undefined) : this.context.innerFrame;
		this.isLayouting = true;
		this.y = Utils.calculateLayoutY(parentFrame, this.constraintValues, this.height);
		return this.isLayouting = false;
	}
	...
	contentFrame() {
		if (!this._children.length) { return {x: 0, y: 0, width: 0, height: 0}; }
	...
	updateForDevicePixelRatioChange() {
		return ["width", "height", "webkitTransform", "boxShadow", "textShadow", "webkitFilter", "borderRadius", "borderWidth", "fontSize", "letterSpacing", "wordSpacing", "textIndent"].map((cssProperty) =>
	...
	screenScaledFrame() {
		...
		for (var parent of Array.from(layers)) {
			...
			var factorX = (left = __guardMethod__(p, 'screenScaleX', o => o.screenScaleX())) != null ? left : 1;
			var factorY = (left1 = __guardMethod__(p, 'screenScaleY', o1 => o1.screenScaleY())) != null ? left1 : 1;
			var layerScaledFrame = (left2 = (typeof parent.scaledFrame === 'function' ? parent.scaledFrame() : undefined)) != null ? left2 : {x: 0, y: 0};
	...
	_createBorderElement() {
		if (this._elementBorder != null) { return; }
	...
	_updateHTMLScale() {
		if ((this._elementHTML == null)) { return; }
		...
		if ((this.htmlIntrinsicSize == null)) {
			...
			this._elementHTML.style.transform = `scale(${(this.context.scale * this.width) / this.htmlIntrinsicSize.width}, ${(this.context.scale * this.height) / this.htmlIntrinsicSize.height})`;
	...
	copy() {
		...
		for (var child of Array.from(this._children)) {
			...
			if (copiedChild !== null) { copiedChild.parent = layer; }
		}
	...
	bringToFront() {
		let maxIndex = null;
		const siblings = (this.parent != null ? this.parent._children : undefined) != null ? (this.parent != null ? this.parent._children : undefined) : this.context._layers;
		if (siblings.count <= 1) { return; }
		for (var layer of Array.from(siblings)) {
			if (layer === this) { continue; }
			if (maxIndex == null) { maxIndex = layer.index; }
	...
	sendToBack() {
		...
		for (var layer of Array.from(siblings)) {
			if (layer === this) { continue; }
			if (minIndex == null) { minIndex = layer.index; }
	...
	emit(eventName, ...args) {
		...
		if (this._cancelClickEventInDragSession && !this._draggable) {
			...
			if ([
				...
				Events.LongPress, Events.LongPressStart, Events.LongPressEnd].includes(eventName)) {
					...
					if (LayerDraggable._globalDidDrag === true) { return; }
				}
		}
		...
		if (((args[0] != null ? args[0].clientX : undefined) != null) || ((args[0] != null ? args[0].clientY : undefined) != null)) {

			const event = args[0];
			let point = {x: event.clientX, y: event.clientY};

			event.point = Utils.convertPointFromContext(point, this, true);
			event.contextPoint = Utils.convertPointFromContext(point, this.context, true);

			if (event.touches != null) {
				for (var touch of Array.from(event.touches)) {
					point = {x: touch.clientX, y: touch.clientY};
					touch.point = Utils.convertPointFromContext(point, this, true);
					touch.contextPoint = Utils.convertPointFromContext(point, this.context, true);
				}
			}
		}
	...
	_addListener(eventName, listener) {
		...
		if (Utils.domValidEvent(this._element, eventName) || (needle = eventName, Array.from(_.values(Gestures)).includes(needle))) {
	...
	onMouseEnter(cb) { return this.on(Events.MouseEnter, cb); }
	onMouseLeave(cb) { return this.on(Events.MouseLeave, cb); }
	...
	_showHint(targetLayer) {
		...
		if (!this.shouldShowHint(targetLayer)) {
			for (var layer of Array.from(this.children)) { layer._showHint(targetLayer); }
			return null;
		}
