...
const Cls = (exports.ScrollComponent = class ScrollComponent extends Layer {
	static initClass() {
	
		// Proxy properties directly from the draggable
		this.define("velocity", this.proxyProperty("content.draggable.velocity", {importable: false, exportable: false}));
		this.define("scrollHorizontal", this.proxyProperty("content.draggable.horizontal"));
		this.define("scrollVertical", this.proxyProperty("content.draggable.vertical"));
		...
		this.define("isDragging", this.proxyProperty("content.draggable.isDragging", {importable: false, exportable: false}));
		this.define("isMoving", this.proxyProperty("content.draggable.isMoving", {importable: false, exportable: false}));
		this.define("isAnimating", this.proxyProperty("content.draggable.isAnimating", {importable: false, exportable: false}));
		this.define("propagateEvents", this.proxyProperty("content.draggable.propagateEvents"));
		this.define("directionLock", this.proxyProperty("content.draggable.directionLock"));
		this.define("directionLockThreshold", this.proxyProperty("content.draggable.directionLockThreshold"));
		...
		this.define("mouseWheelSpeedMultiplier", this.simpleProperty("mouseWheelSpeedMultiplier", 1));
		...
		this.define("scroll", {
			...
			set(value) {
				...
				if (value === false) { this.content.animateStop(); }
		...
		this.define("scrollX", {
			...
			set(value) {
				...
				return this.content.x = this._calculateContentPoint({x: value, y: 0}).x;
			}
		}
		);
		...
		this.define("scrollY", {
			...
			set(value) {
				if (!this.content) { return; }
				this.content.draggable.animateStop();
				return this.content.y = this._calculateContentPoint({x: 0, y: value}).y;
			}
		}
		);
	...
	calculateContentFrame() {
		...
		let size;
		if (!this.content) { return Utils.rectZero(); }
	...
	updateContent() {
		...
		constraintsFrame = {
			...
			width: ((constraintsFrame.width  + constraintsFrame.width)  - this.width) +
				this._contentInset.left + this._contentInset.right,
			height: ((constraintsFrame.height + constraintsFrame.height) - this.height) +
				this._contentInset.top + this._contentInset.bottom
		};
		...
		if (this.content.children.length) {
			if (this.content.backgroundColor != null ? this.content.backgroundColor.isEqual(Framer.Defaults.Layer.backgroundColor) : undefined) {
	...
	scrollToPoint(point, animate, animationOptions) {
		...
		if (animationOptions == null) { animationOptions = {curve: "spring(500, 50, 0)"}; }
		const contentPoint = this._calculateContentPoint(point);
		this.content.draggable.animateStop();

		if (animate) {
			point = {};
			if (contentPoint.hasOwnProperty("x")) { point.x = contentPoint.x; }
			if (contentPoint.hasOwnProperty("y")) { point.y = contentPoint.y; }
	...
	scrollToClosestLayer(originX, originY, animate, animationOptions) {
		...
		if (animate == null) { animate = true; }
		if (animationOptions == null) { animationOptions = {curve: "spring(500, 50, 0)"}; }
		const closestLayer = this.closestContentLayer(originX, originY, animate, animationOptions);
		if (closestLayer) {
			this.scrollToLayer(closestLayer, originX, originY);
			return closestLayer;
		} else {
			if (!closestLayer) { this.scrollToPoint({x: 0, y: 0}); }
			return null;
		}
	}
	...
	closestContentLayer(originX, originY) {
		if (originX == null) { originX = 0; }
		if (originY == null) { originY = 0; }
		const scrollPoint = Utils.framePointForOrigin(this.scrollFrame, originX, originY);
	...
	_pointInConstraints(point) {
		...
		const {minX, maxX, minY, maxY} = this.content.draggable.
			_calculateConstraints(this.content.draggable.constraints);
	...
	addListener(...args) {
		const adjustedLength = Math.max(args.length, 1), eventNames = args.slice(0, adjustedLength - 1), listener = args[adjustedLength - 1];
		super.addListener(...arguments);
		return (() => {
			const result = [];
			for (var eventName of Array.from(eventNames)) {
				var needle;
				if ((needle = eventName, Array.from(_.keys(EventMappers)).includes(needle))) { result.push(this.content.on(EventMappers[eventName], listener)); } else {
	...
	removeListener(...args) {
		...
		return (() => {
			...
			for (var eventName of Array.from(eventNames)) {
				var needle;
				if ((needle = eventName, Array.from(_.keys(EventMappers)).includes(needle))) { result.push(this.content.off(EventMappers[eventName], listener)); } else {
					result.push(undefined);
				}
			}
			return result;
	...
	_onMouseWheel(event) {
		...
		const {minX, maxX, minY, maxY} = this.content.draggable._calculateConstraints(
...
var wrapComponent = function(instance, layer, options) {
	...
	if (options == null) { options = {correct: true}; }
	...
	if (scroll.__framerInstanceInfo == null) { scroll.__framerInstanceInfo = {}; }
