...
const centerLayer = function(layer, snapToPixels) {
	if (snapToPixels == null) { snapToPixels = false; }
	const {
        frame
    } = layer;
	if (layer.parent) {
...
const Cls = (exports.DeviceComponent = class DeviceComponent extends BaseClass {
	static initClass() {
			...
			set(deviceType) {
				...
				if (_.isString(deviceType)) {
					...
					for (var key of Array.from(_.keys(Devices))) {
						...
						if (lDevicetype === lKey) { device = Devices[key]; }
					}
				}
				...
				if (device.backgroundColor != null) { this.screen.backgroundColor = device.backgroundColor; }
		...
		this.define("hideBezel", {
			get() {
				if (this._forceHideBezel) { return true; }
				if (!Utils.isFramerStudio() && this._forceHide) { return false; }
				return (this._hideBezel != null ? this._hideBezel : false);
			},
			set(hideBezel) {
				if (!Utils.isFramerStudio()) { return; }
		...
		this.define("deviceScale", {
			...
			set(deviceScale) { return this.setDeviceScale(deviceScale, false); }
		}
		);
		...
		this.define("contentScale", {
			get() { return this._contentScale || 1; },
			set(contentScale) { return this.setContentScale(contentScale, false); }
		}
		);
		...
		this.define("orientation", {
			get() {
				if (Utils.isMobile()) { return window.orientation; }
				return this._orientation || 0;
			},
	
			set(orientation) { return this.setOrientation(orientation, false); }
		}
		);
	
		this.define("isPortrait", {get() { return (Math.abs(this.orientation) % 180) === 0; }});
		this.define("isLandscape", {get() { return !this.isPortrait; }});
		...
		this.define("orientationName", {
			...
			set(orientationName) { return this.setOrientation(orientationName, false); }
		}
		);
	}
	...
	constructor(options) {
		...
		this._viewportOrientationOffset = this._viewportOrientationOffset.bind(this);
	...
	_setup() {
		...
		this.handsImageLayer = new Layer({parent: this.hands, name: "handsImage"});
		...
		this.screenBackground = new Layer({parent: this.hands, name: "screenBackground", backgroundColor: "black"});
		...
		this.screenMask = new Layer({parent: this.screen, name: "mask", backgroundColor: null});
		...
		if (!Utils.isMobile()) { Framer.CurrentContext.domEventManager.wrap(window).addEventListener("resize", this._update); }
		if (Utils.isMobile()) { Framer.CurrentContext.domEventManager.wrap(window).addEventListener("resize", this._orientationChange); }

		// This avoids rubber banding on mobile
		for (var layer of [this.background, this.phone, this.viewport, this.content, this.screen, this.screenMask]) {
		...
		this._context = new Framer.Context({parent: this.content, name: "DeviceScreen"});
	...
	_update() {
		...
		if (contentScaleFactor > 1) { contentScaleFactor = 1; }
		...
		if (this._shouldRenderFullScreen()) {
			...
			if (Utils.isMobile()) {
				...
				if (((clientWidth < clientHeight) && (width > height)) || ((clientWidth > clientHeight) && (width < height))) {
			...
			screenSizeChanged = (this.content.width !== width) || (this.content.height !== height);
			for (var layer of [this.background, this.hands, this.phone, this.viewport, this.content, this.screen, this.screenMask]) {
				layer.x = (layer.y = 0);
				layer.width = width;
				layer.height = height;
				layer.scale = 1;
			...
			if ((this.deviceType !== "fullscreen") || Utils.isMobile()) {
				screenSizeChanged = screenSizeChanged || (this._context.devicePixelRatio !== window.devicePixelRatio);
			...
			[width, height] = Array.from(this._getOrientationDimensions(
				this._device.screenWidth / contentScaleFactor,
				this._device.screenHeight / contentScaleFactor));

			this.screenMask.width = (this.screen.width = (this.viewport.width = this._device.screenWidth));
			this.screenMask.height = (this.screen.height = (this.viewport.height = this._device.screenHeight));
			screenSizeChanged = (this.content.width !== width) || (this.content.height !== height);
			this.content.width  = width;
			this.content.height = height;
			this.screenBackground.width = this.screen.width + 40;
			this.screenBackground.height = this.screen.height + 40;
			if (this.selectedHand && (this._orientation === 0)) { this.setHand(this.selectedHand); }
			centerLayer(this.screenBackground, true);
			centerLayer(this.screen, true);
			centerLayer(this.screenMask, true);

			const pixelRatio = this._device.devicePixelRatio != null ? this._device.devicePixelRatio : 1;
			screenSizeChanged = screenSizeChanged || (this._context.devicePixelRatio !== pixelRatio);
	...
	_deviceImageUrl(name) {
		...
		if (!Array.from(BuiltInDevices).includes(this._deviceType) || (this._deviceType === "custom")) {
			return name;
		}
	...
	forceHideBezel(forceHide) {
		if (forceHide == null) { forceHide = true; }
	...
	_calculatePhoneScale(windowWidth, windowHeight) {
		...
		const [width, height] = Array.from(this._getOrientationDimensions(dimension.width, dimension.height));

		if (this.hideBezel) {
			padding = 0;
		} else {
			const paddingOffset = (this._device != null ? this._device.paddingOffset : undefined) || 0;
			padding = (this.padding + paddingOffset) * 2;
		}

		if (windowWidth == null) { windowWidth = window.innerWidth; }
		if (windowHeight == null) { windowHeight = window.innerHeight; }
		...
		if (phoneScale < (1 / 64.0)) { phoneScale = 1 / 64.0; }
		...
		if ((phoneScale > 1) && !this.hideBezel) { phoneScale = 1; }
	...
	setOrientation(orientation, animate, suggestedSize=null) {
		...
		let scale;
		if (animate == null) { animate = false; }
		if (Utils.framerStudioVersion() === oldDeviceMaxVersion) { orientation *= -1; }
		...
		if (suggestedSize != null) {
			scale = this._calculatePhoneScale(suggestedSize.width, suggestedSize.height);
		...
		if (animate) {
			...
			const animation = this.hands.animate(_.extend(options,
				{properties: handsProperties})
			);
			this.viewport.animate(_.extend(options,
			...
			animation.on(Events.AnimationStop, () => {
				...
				return this.screenBackground.visible = previousScreenBackgroundVisiblity;
			});
		...
		if (this._orientation !== 0) { this.handsImageLayer.image = ""; }
	...
	_viewportOrientationOffset() {
		...
		const [width, height] = Array.from(this._getOrientationDimensions(this._device.screenWidth, this._device.screenHeight));

		this.content.width = width;
		this.content.height = height;

		let offset = (this.screen.width - width) / 2;
		if (this._orientation === -90) { offset *= -1; }
	...
	_getOrientationDimensions(width, height) {
		if (this.isLandscape) { return [height, width]; } else { return [width, height]; }
	}
	...
	setHand(hand) {
		...
		if (!hand || !this.handSwitchingSupported()) { return this.handsImageLayer.image = ""; }
		...
		if (handData) {
			...
			if (handData.offset) { this.handsImageLayer.y = handData.offset; }
...
const old_Nexus5BaseDeviceHand = _.extend({}, old_Nexus5BaseDevice, { // 2692 × 2996
