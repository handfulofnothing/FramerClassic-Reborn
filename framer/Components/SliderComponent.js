...
const Cls = (exports.SliderComponent = class SliderComponent extends Layer {
	static initClass() {
		...
		this.define("knobSize", {
			...
			set(value) {
				...
				if (isRound) { this.knob.borderRadius = this._knobSize / 2; }
	...
	_updateValue() {
		...
		if (this._lastUpdatedValue === this.value) { return; }
	...
	animateToValue(value, animationOptions) {
		if (animationOptions == null) { animationOptions = {curve: "spring(300, 25, 0)"}; }
		if (!_.isFinite(value)) { return; }
		if (this.width > this.height) {
			animationOptions.properties = {x: this.pointForValue(value) - (this.knob.width/2)};
		} else {
			animationOptions.properties = {y: this.pointForValue(value) - (this.knob.height/2)};
		}
