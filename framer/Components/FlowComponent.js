...
const FlowComponentLayerScrollKey = "_flowComponentWrapped";
...
const Cls = (exports.FlowComponent = class FlowComponent extends Layer {
	static initClass() {
		...
		this.define("stack",
			{get() { return this._stack.map(item => item.layer); }});
	
		this.define("current",
			{get() { return __guard__(this._stack[this._stack.length - 1], x => x.layer); }});
	
		this.define("previous",
			{get() { return __guard__(this._stack[this._stack.length - 2], x => x.layer); }});
	
		this.define("scroll",
			{get() { return (this.current != null ? this.current._flowScroll : undefined); }});
	
		this.define("mouseWheelEnabled", {
			default: false,
			get() { return this._mouseWheelEnabled; },
			set(value) {	return this._mouseWheelEnabled = value; }
		}
		);
		...
		this.define("header", {
			...
			set(layer) {
				...
				if (this.current) { return this._wrapLayer(this.current); }
			}
		}
		);
	...
	constructor(layerOrOptions, options) {
		...
		if (layerOrOptions == null) { layerOrOptions = {}; }
		...
		if (!options.size) {
			if (options.width == null) { options.width = Screen.width; }
			if (options.height == null) { options.height = Screen.height; }
		}

		if (options.clip == null) { options.clip = true; }
	...
	reset() {
		...
		if (this._stack) {
			for (var item of Array.from(this._stack)) {
				if (item.layer !== this._initial) { item.layer.visible = false; }
			}
		}
		...
		if (this._initial) { return this.showNext(this._initial, {animate: false}); }
	}
	...
	transition(layer, transitionFunction, options) {
		...
		if (options == null) { options = {}; }
		if (!(layer instanceof Layer)) { throw new Error("FlowComponent.transition expects a layer"); }
		if (!transitionFunction) { throw new Error("FlowComponent.transition expects transitionFunction"); }
		...
		if (options.scroll && options.wrap) { wrappedLayer = this._wrapLayer(layer); }
	...
	showNext(layer, options) {
		if (options == null) { options = {}; }
		if (this._initial == null) { this._initial = layer; }
	...
	showPrevious(options) {
		...
		if (options.count > 1) {
			...
			for (let n = 2, end = count, asc = 2 <= end; asc ? n <= end : n >= end; asc ? n++ : n--) { this.showPrevious({animate: false, count: 1}); }
		}

		const previous = this._stack.pop();
		return this._runTransition(previous != null ? previous.transition : undefined, "back", options.animate, this.current, previous.layer);
	}
	...
	_showOverlay(layer, transition, options) {
		if (options == null) { options = {}; }
		return this.transition(layer, transition, _.defaults({}, options,
			{animate: true, scroll: false, modal: false}));
	}

	_handleOverlayTap() {
		if (!this.isModal) { return this.showPrevious(); }
	}
	...
	_wrapLayer(flowLayer) {
		...
		if (flowLayer instanceof ScrollComponent) { return flowLayer; }
		if (flowLayer._flowWrapped) { return flowLayer; }
		...
		if ((flowLayer !== layer) &&
		   ((previousWrappingContent != null ? previousWrappingContent.children.length : undefined) === 0) &&
		   ((previousWrappingScroll != null ? previousWrappingScroll.children.length : undefined) === 1) &&
		   ((previousWrappingScroll != null ? previousWrappingScroll.children[0] : undefined) === previousWrappingContent)) {
		...
		for (scroll of [layer, ...Array.from(layer.children)]) {
			...
			if (scroll instanceof ScrollComponent) {
				var inset = {};
				if (scroll.y === 0) { inset.top = (this.header != null ? this.header.height : undefined) || 0; }
				if (scroll.maxY === this.height) { inset.bottom = (this.footer != null ? this.footer.height : undefined) || 0; }
	...
	_forwardScrollEvents(scroll) {
		...
		if (!(scroll instanceof ScrollComponent)) { return; }
		if (scroll._flowForward === true) { return; }
	...
	_buildTransition(template, layerA, layerB, overlay) {
		...
		transition.forward = (animate, callback) => {
			...
			const forwardEvents = (group, direction) => {
				group.once(Events.AnimationHalt, () => this.emit(Events.TransitionHalt, layerA, layerB, direction));
				group.once(Events.AnimationStop, () => this.emit(Events.TransitionStop, layerA, layerB, direction));
				return group.once(Events.AnimationEnd, () => this.emit(Events.TransitionEnd, layerA, layerB, direction));
			};
			...
			if (overlay && template.overlay) {
				if (this.header) { this.header.placeBehind(overlay); }
				if (this.footer) { this.footer.placeBehind(overlay); }
			} else {
				if (this.header) { this.header.bringToFront(); }
				if (this.footer) { this.footer.bringToFront(); }
			}
		...
		transition.back = (animate, callback) => {
			...
			if (animate == null) { animate = true; }
			const forwardEvents = (group, direction) => {
				group.once(Events.AnimationHalt, () => this.emit(Events.TransitionHalt, layerB, layerA, direction));
				group.once(Events.AnimationStop, () => this.emit(Events.TransitionStop, layerB, layerA, direction));
				return group.once(Events.AnimationEnd, () => this.emit(Events.TransitionEnd, layerB, layerA, direction));
			};
	...
	onTransitionStart(cb) { return this.on(Events.TransitionStart, cb); }
	onTransitionHalt(cb) { return this.on(Events.TransitionHalt, cb); }
	onTransitionStop(cb) { return this.on(Events.TransitionStop, cb); }
	onTransitionEnd(cb) { return this.on(Events.TransitionEnd, cb); }

	onStart(cb) { return this.onTransitionStart(cb); }
	onHalt(cb) { return this.onTransitionHalt(cb); }
	onStop(cb) { return this.onTransitionStop(cb); }
	onEnd(cb) { return this.onTransitionEnd(cb); }
});
Cls.initClass();
...
const findPossibleHeader = function(layer) {
	...
	for (child of Array.from(layer.children)) {
		var attachedLeft = (child.x === 0) || ((child.constraintValues != null ? child.constraintValues.left : undefined) === 0);
		var atttachedRight = (child.width === layer.width) || ((child.constraintValues != null ? child.constraintValues.right : undefined) === 0);
		var attachedTop = (child.y === 0) || ((child.constraintValues != null ? child.constraintValues.top : undefined) === 0);
	...
	for (child of Array.from(layer.children)) {
		...
		if (child.minY < candidate.maxY) { return; }
	}
...
const findPossibleFooter = function(layer) {
	...
	for (child of Array.from(layer.children)) {
		var attachedLeft = (child.x === 0) || ((child.constraintValues != null ? child.constraintValues.left : undefined) === 0);
		var atttachedRight = (child.width === layer.width) || ((child.constraintValues != null ? child.constraintValues.right : undefined) === 0);
		var attachedBottom = (child.maxY === layer.height) || ((child.constraintValues != null ? child.constraintValues.bottom : undefined) === 0);
	...
	for (child of Array.from(layer.children)) {
		if (candidate === child) { continue; }
		if (child.maxY > candidate.minY) { return; }
	}
...
const findHeader = function(layer) {
	...
	if (header && footer) {
		if (header.maxY === footer.minY) {
			if (header.height >= footer.height) { return; }
		}
	}
...
const findFooter = function(layer) {
	...
	if (header && footer) {
		if (header.maxY === footer.minY) {
			if (footer.height >= header.height) { return; }
		}
	}
...
const findBody = function(layer, header, footer) {
	...
	for (var child of Array.from(layer.children)) {
		...
		if ((child.x === 0) && (child.width === layer.width)) {
			if (header && footer && (child.minY === header.maxY) && (child.maxY === footer.minY)) {
				return child;
			} else if (header && (child.minY === header.maxY) && (child.maxY === layer.height)) {
				return child;
...
const guessBodyFrame = function(layer, header, footer) {
	...
	let frame;
	if (!header && !footer) { return; }
	if ((header != null ? header.maxY : undefined) === (footer != null ? footer.minY : undefined)) { return; }
	...
	if (((header != null ? header.height : undefined) || 0) > frame.height) { return; }
	if (((footer != null ? footer.height : undefined) || 0) > frame.height) { return; }
...
var layoutPage = function(layer, size) {
		...
		if (bodyFrame) {
			...
			for (var child of Array.from(layer.children)) {
				...
				if (child.constraintValues != null) {
					if ((child.constraintValues != null ? child.constraintValues.top : undefined) != null) {
						if (child.constraintValues != null) {
	...
	if (!body) { return layer; }

	bodyFrame = body.frame;
	bodyFrame.width = size.width;
	bodyFrame.height = size.height - ((header != null ? header.height : undefined) || 0) - ((footer != null ? footer.height : undefined) || 0);
...
Transitions.show = function(nav, layerA, layerB, overlay) {
	...
	return transition = {
		layerA: {
			...
			hide: {options, x: 0 - ((layerA != null ? layerA.width : undefined) / 2), y: 0}
		},
		layerB: {
			show: {options, x: 0, y: 0},
...
Transitions.overlayLeft = function(nav, layerA, layerB, overlay) {
	...
	return transition = {
		layerB: {
			...
			hide: {options: {curve: "spring(300, 35, 0)"}, y: 0, x: 0 - (layerB != null ? layerB.width : undefined)}
		},
		overlay: {
			show: {options: {time: 0.1}, opacity: .5, x: 0, y: 0, size: nav.size},
			hide: {options: {time: 0.1}, opacity: 0, x: 0, y: 0, size: nav.size}
...
Transitions.overlayRight = function(nav, layerA, layerB, overlay) {
	...
	return transition = {
		layerB: {
			show: {options: {curve: "spring(300, 35, 0)"}, y: 0, x: (nav != null ? nav.width : undefined) - (layerB != null ? layerB.width : undefined)},
			hide: {options: {curve: "spring(300, 35, 0)"}, y: 0, x: (nav != null ? nav.width : undefined)}
		},
		overlay: {
			show: {options: {time: 0.1}, opacity: .5, x: 0, y: 0, size: nav.size},
			hide: {options: {time: 0.1}, opacity: 0, x: 0, y: 0, size: nav.size}
...
Transitions.overlayBottom = function(nav, layerA, layerB, overlay) {
	...
	return transition = {
		layerB: {
			show: {options: {curve: "spring(300, 35, 0)"}, x: Align.center, y: (nav != null ? nav.height : undefined) - (layerB != null ? layerB.height : undefined)},
			hide: {options: {curve: "spring(300, 35, 0)"}, x: Align.center, y: (nav != null ? nav.height : undefined)}
		},
		overlay: {
			show: {options: {time: 0.1}, opacity: .5, x: 0, y: 0, size: nav.size},
			hide: {options: {time: 0.1}, opacity: 0, x: 0, y: 0, size: nav.size}
