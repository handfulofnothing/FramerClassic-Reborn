...
const {ScrollComponent} = require("./ScrollComponent");
...
const Cls = (exports.PageComponent = class PageComponent extends ScrollComponent {
	static initClass() {
	
		this.define("originX", this.simpleProperty("originX", .5));
		this.define("originY", this.simpleProperty("originY", .5));
		this.define("velocityThreshold", this.simpleProperty("velocityThreshold", 0.1));
	
		this.define("closestPage",  {get() { return this.closestContentLayerForScrollPoint(this._originScrollPoint(), this.originX, this.originY); }});
		this.define("currentPage",  {get() { return _.last(this._previousPages); }});
		this.define("previousPage", {get() { return this._previousPages[this._previousPages.length-2]; }});
	}
	...
	nextPage(direction, currentPage=null, withoutCurrentPage) {
		...
		if (withoutCurrentPage == null) { withoutCurrentPage = true; }
		if (currentPage == null) { ({
            currentPage
        } = this); }
		...
		let point = {x: 0, y: 0};
		if (currentPage) { point = Utils.framePointForOrigin(currentPage, this.originX, this.originY); }
		if (!withoutCurrentPage) {
			point = {x: this.scrollX + (this.originX * this.width), y: this.scrollY + (this.originY * this.height)};
		}

		if (["up", "top", "north"].includes(direction)) { layers = this.content.childrenAbove(point, this.originX, this.originY); }
		if (["down", "bottom", "south"].includes(direction)) { layers = this.content.childrenBelow(point, this.originX, this.originY); }
		if (["left", "west"].includes(direction)) { layers = this.content.childrenLeft(point, this.originX, this.originY); }
		if (["right", "east"].includes(direction)) { layers = this.content.childrenRight(point, this.originX, this.originY); }
	...
	snapToNextPage(direction, animate, animationOptions=null) {
		...
		if (nextPage == null) { nextPage = this.closestPage; }
	...
	snapToPreviousPage(animate, animationOptions=null) {
		...
		return this._previousPages = this._previousPages.slice(0, +(this._previousPages.length-3) + 1 || undefined);
	}
	...
	addPage(page, direction) {
		...
		if (this.content.children.length) {
			if (["right", "east"].includes(direction)) { point.x = Utils.frameGetMaxX(this.content.contentFrame()); }
			if (["down", "bottom", "south"].includes(direction)) { point.y = Utils.frameGetMaxY(this.content.contentFrame()); }
		}
	...
	_scrollMove() {
		...
		if ((needle = currentPage, ![_.last(this._previousPages), undefined].includes(needle))) {
			this._previousPages.push(currentPage);
			return this.emit("change:currentPage", {old: this.previousPage, new: currentPage});
		}
	}

	_scrollEnd() {

		if (this.content.isAnimating) { return; }
		...
		const xDisabled = !this.scrollHorizontal && ((this.direction === "right") || (this.direction === "left"));
		const yDisabled = !this.scrollVertical && ((this.direction === "down") || (this.direction === "up"));

		const xLock = this.content.draggable._directionLockEnabledX && ((this.direction === "right") || (this.direction === "left"));
		const yLock = this.content.draggable._directionLockEnabledY && ((this.direction === "down") || (this.direction === "up"));

		const maximumVelocity = Math.max(Math.abs(velocity.x), Math.abs(velocity.y));
