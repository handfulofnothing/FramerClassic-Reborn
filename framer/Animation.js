...
const {LinearAnimator} = require("./Animators/LinearAnimator");
const {SVG} = require("./SVG");
const Curves = require("./Animators/Curves");

const numberRE = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/;
const relativePropertyRE = new RegExp("^(?:([+-])=|)(" + numberRE.source + ")([a-z%]*)$", "i");

const isRelativeProperty = v => _.isString(v) && relativePropertyRE.test(v);

const evaluateRelativeProperty = function(target, k, v) {
	const [match, sign, number, unit, ...rest] = Array.from(relativePropertyRE.exec(v));
	if (sign) { return target[k] + ((sign + 1) * number); }
	return +number;
};

const Cls = (exports.Animation = class Animation extends BaseClass {
	static initClass() {
		...
		this.define("isAnimating",
			{get() { let needle;
			return (needle = this, Array.from(this.layer.animations()).includes(needle)); }});
		...
		return (() => {
			...
			for (var k in this._stateB) {
				...
				if (SVG.isPath(v)) {
					...
					var end = null;
					switch (k) {
						case "x": case "minX": case "midX": case "maxX": case "width":
							direction = "horizontal";
							break;
						case "y": case "minY": case "midY": case "maxY": case "height":
							direction = "vertical";
							break;
						case "rotation": case "rotationZ": case "rotationX": case "rotationY":
							direction = "angle";
							break;
					}

					result.push(this._valueUpdaters[k] = path.valueUpdater(direction, this._target, this._target[k]));
				} else if (Color.isColorObject(v) || Color.isColorObject(this._stateA[k])) {
					result.push(this._valueUpdaters[k] = this._updateColorValue);
				} else if (Gradient.isGradient(v) || Gradient.isGradient(this._stateA[k])) {
					...
					result.push(this._stateA[k] != null ? this._stateA[k] : (this._stateA[k] = Gradient.multiplyAlpha(v, 0)));
				} else if (k === "borderWidth") {
					result.push(this._valueUpdaters[k] = this._updateNumericObjectValue.bind(this, ["top", "left", "bottom", "right"]));
				} else if (k === "borderRadius") {
					result.push(this._valueUpdaters[k] = this._updateNumericObjectValue.bind(this, ["topLeft", "topRight", "bottomRight", "bottomLeft"]));
				} else if (k === "template") {
	...
	_updateValues(value) {
		...
		for (var k in this._stateB) { var v = this._stateB[k]; this._valueUpdaters[k](k, value, delta); }
		return null;
	}
	...
	_interpolateNumericObjectValues(propKeys, valueA, valueB, value, flatten) {
		...
		for (var propKey of Array.from(propKeys)) {
			var keyValueA = _.isNumber(valueA) ? valueA : (valueA != null ? valueA[propKey] : undefined);
			var keyValueB = _.isNumber(valueB) ? valueB : (valueB != null ? valueB[propKey] : undefined);
			// If the key value is undefined in one state, use the value from the other
			if (keyValueA == null) { keyValueA = keyValueB; }
			if (keyValueB == null) { keyValueB = keyValueA; }
	...
	_calculateNumericObjectValue(propKeys, key, value, flatten) {
		...
		return this._interpolateNumericObjectValues(propKeys, valueA, valueB, value, flatten);
	}

	_updateNumericObjectValue(propKeys, key, value, flatten) {
		if (flatten == null) { flatten = true; }
		const result = this._calculateNumericObjectValue(propKeys, key, value, flatten);
	...
	_updateGradientValue(key, value) {
		...
		const gradientB = Gradient._asPlainObject(this._stateB[key] != null ? this._stateB[key] : Gradient.multiplyAlpha(gradientA, 0));
	...
	_updateShadows(key, value) {
		...
		const result = [];
		const numShadows = Math.max((this._stateA[key] != null ? this._stateA[key].length : undefined) != null ? (this._stateA[key] != null ? this._stateA[key].length : undefined) : 0, (this._stateB[key] != null ? this._stateB[key].length : undefined) != null ? (this._stateB[key] != null ? this._stateB[key].length : undefined) : 0);
		for (let index = 0, end = numShadows, asc = 0 <= end; asc ? index < end : index > end; asc ? index++ : index--) {
			var left;
			var fromShadow = this._stateA[key] != null ? this._stateA[key][index] : undefined;
			var toShadow = this._stateB[key] != null ? this._stateB[key][index] : undefined;
			if ((toShadow == null) && (fromShadow == null)) {
				continue;
			}
			var type = (left = (toShadow != null ? toShadow.type : undefined) != null ? (toShadow != null ? toShadow.type : undefined) : (fromShadow != null ? fromShadow.type : undefined)) != null ? left : Framer.Defaults.Shadow.type;
			if (fromShadow == null) { fromShadow = _.defaults({color: null, type}, Framer.Defaults.Shadow); }
			if (toShadow == null) { toShadow = _.defaults({color: null, type}, Framer.Defaults.Shadow); }
			_.defaults(fromShadow, Framer.Defaults.Shadow);
			_.defaults(toShadow, Framer.Defaults.Shadow);
			result[index] = this._interpolateNumericObjectValues(["x", "y", "blur", "spread"], fromShadow, toShadow, value, false);
			result[index].color = Color.mix(fromShadow.color, toShadow.color, value, false, this.options.colorModel);
	...
	_updateTemplateValue(key, value) {
		...
		if (!_.isObject(toData)) {
			k = this._target._styledText != null ? this._target._styledText.buildTemplate() : undefined;
			if (!k) { return; }
			valueB = toData;
			if (_.isNumber(valueB)) {
				valueA = _.isObject(fromData) ? fromData[k] : fromData;
				if (!_.isNumber(valueA)) { valueA = 0; }
	...
	static isAnimatable(v) {
		return _.isNumber(v) || _.isFunction(v) || isRelativeProperty(v) || Color.isColorObject(v) || Gradient.isGradientObject(v) || SVG.isPath(v);
	}

	// Special cases that animate with different types of objects
	static isAnimatableKey(k) {
		return ["gradient", "borderWidth", "borderRadius", "template", "shadows"].includes(k);
	}
	...
	static filterAnimatableProperties(properties, layer) {
		...
		for (var k in properties) {
			var matches;
			var v = properties[k];
			if (["frame", "size", "point", "midPoint", "path"].includes(k)) { // Derived properties
				...
				switch (k) {
					...
					case "midPoint": derivedKeys = ["midX", "midY"]; break;
					case "path": derivedKeys = ["x", "y", "rotation"]; break;
					default: derivedKeys = [];
				}
				if (SVG.isPath(v)) {
					if (k === "path") {
				...
				if (animatableProperties.shadows == null) { var left;
				animatableProperties.shadows = (left = _.clone(layer.shadows)) != null ? left : []; }
