...
const {computeDerivedCurveOptions, computeDuration, computeDampingRatio} = require("./SpringCurveValueConverter");
const {SpringRK4Animator} = require("./SpringRK4Animator");
const {Defaults} = require("../Defaults");

const Bezier = (...values) => (function(options) {
...
const BezierDefaults = {
	linear(options) { if (options == null) { options = {}; } return Bezier(0, 0, 1, 1)(options); },
	ease(options) { if (options == null) { options = {}; } return Bezier(.25, .1, .25, 1)(options); },
	easeIn(options) { if (options == null) { options = {}; } return Bezier(.42, 0, 1, 1)(options); },
	easeOut(options) { if (options == null) { options = {}; } return Bezier(0, 0, .58, 1)(options); },
	easeInOut(options) { if (options == null) { options = {}; } return Bezier(.42, 0, .58, 1)(options); }
};
...
const Spring = function(dampingRatio, mass, velocity) {
	...
	return function(options) {
		...
		if (curveOptions.dampingRatio != null) {
			duration = (options != null ? options.time : undefined) != null ? (options != null ? options.time : undefined) : 1;
			const derivedOptions = computeDerivedCurveOptions(curveOptions.dampingRatio, duration, curveOptions.velocity, curveOptions.mass);
...
exports.fromString = function(string) {
	...
	switch (func.name) {
		case "linear": return Bezier.linear;
		case "ease": return Bezier.ease;
		case "ease-in": return Bezier.easeIn;
		case "ease-out": return Bezier.easeOut;
		case "ease-in-out": return Bezier.easeInOut;
		case "bezier-curve": case "cubic-bezier":
			return Bezier(...Array.from(args || []));
		case "spring": case "spring-rk4": case "spring-dho":
			var pairs = _.zipWith(["tension", "friction", "velocity", "tolerance"], args, [250, 25, 0, 1 / 100], (key, value, defaults) => [key, value != null ? value : defaults]);
